...
def balance(config):
    ...
    if len(normalized) == config['n_class']:
        return tensor(normalized).float()
    else:
        return None
...
def overlay(label_dir, pred_dir, save_dir, class_num, mask=False):
    ...
    if isinstance(mask, np.ndarray): #If applying mask
        ...
        if len([f for f in os.listdir(label_dir) if not f[0] == '.']) != len([f for f in os.listdir(pred_dir) if f[-3:] == 'png']):
            print('Label and Prediction Directories Have Different Number of Files!')
            sys.exit(1)

        # Get lists of label names and prediction names. // Only image file in prediction folder
        label_names = sorted([f for f in os.listdir(label_dir) if not f[0] == '.'])
        pred_names = sorted([f for f in os.listdir(pred_dir) if f[-3:] == 'png'])
    ...
    for file_num in range(len(label_names)):
        ...
        if isinstance(mask, np.ndarray):
            ...
            prediction = Image.open(os.path.join(pred_dir, sorted([f for f in os.listdir(pred_dir) if f[-3:] == 'png'])[file_num]))
        ...
        if class_num == 2:
            ...
            # Follow same procedure with masked region
            if isinstance(mask, np.ndarray):
