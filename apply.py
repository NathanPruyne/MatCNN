...
def average(root_dir):
    ...
    for pic in tqdm(os.listdir(root_dir), desc='Normalizing....', unit=' images'):
...
def sizing(directory):
    ...
    size_list = []
    for image in tqdm(os.listdir(directory), desc='Initializing...', unit=' images'):
        try:
...
def apply(arguments):
    ...
    # The image size tuple must be reversed when used for segmentation
